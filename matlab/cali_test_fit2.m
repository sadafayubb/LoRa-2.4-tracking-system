% Calibration with Polynomial Fit - CaliTest2 (CR 4/8)
% Fits polynomial: measured = f(true_dist)

function cali_test_fit2()
    ...
    % Evaluate fit
    x_fit = linspace(min(true_dist), max(true_dist), 300);
    y_fit = f_poly(x_fit);
    ...
    title(sprintf('Calibration Fit CR 4/8 (Degree %d)', poly_degree));
    grid on;
    legend('Location', 'northwest');
    hold off;
    
    % Save calibration fit plot
    output_file1 = fullfile(figures_dir, 'calibration_fit_cr48.jpg');
    ...
    title('Residuals of Polynomial Fit CR 4/8');
    grid on;
    
    % Save residuals plot
    output_file2 = fullfile(figures_dir, 'residuals_cr48.jpg');
    exportgraphics(fig2, output_file2, 'Resolution', 150);
    fprintf('Saved: %s\n', output_file2);
    ...
    lut_file = fullfile(lut_dir, 'lut_cr48.txt');
    fid = fopen(lut_file, 'w');
    fprintf(fid, 'Calibration LUT - CR 4/8\n');
    ...
    lut_mat_file = fullfile(lut_dir, 'calibration_lut_cr48.mat');
