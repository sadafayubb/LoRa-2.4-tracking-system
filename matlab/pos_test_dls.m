% Position estimation using Direct Linear Solve (DLS) trilateration
% No calibration applied

function pos_test_dls()
    ...
    % Number of test positions (3 rows per test)
    ...
    for i = 1:nTests
        ...
        % Get average distance from each anchor
        r = zeros(3,1);
        for a = 1:3
            meas = table2array(data(rowIdx + a - 1, 5:end));
            r(a) = mean(meas);
        end
        
        % Direct Linear Solve
        A1 = anchors(1,1); B1 = anchors(1,2);
        A2 = anchors(2,1); B2 = anchors(2,2);
        A3 = anchors(3,1); B3 = anchors(3,2);
        R1 = r(1); R2 = r(2); R3 = r(3);
        
        D2 = [-2*(A2 - A1), -2*(B2 - B1)];
        C2 = (R2^2 - R1^2) - (A2^2 - A1^2) - (B2^2 - B1^2);
        D3 = [-2*(A3 - A1), -2*(B3 - B1)];
        C3 = (R3^2 - R1^2) - (A3^2 - A1^2) - (B3^2 - B1^2);
    ...
    % Print results
    fprintf('\n=== DLS Trilateration (No Calibration) ===\n');
    for i = 1:nTests
    ...
    for i = 1:nTests
        ...
        for m = 1:10
            R1 = meas1(m); R2 = meas2(m); R3 = meas3(m);
            D2 = [-2*(anchors(2,1) - anchors(1,1)), -2*(anchors(2,2) - anchors(1,2))];
            C2 = (R2^2 - R1^2) - (anchors(2,1)^2 - anchors(1,1)^2) - (anchors(2,2)^2 - anchors(1,2)^2);
            D3 = [-2*(anchors(3,1) - anchors(1,1)), -2*(anchors(3,2) - anchors(1,2))];
            C3 = (R3^2 - R1^2) - (anchors(3,1)^2 - anchors(1,1)^2) - (anchors(3,2)^2 - anchors(1,2)^2);
    ...
    legend([h1, h2, h3, h4], {'True Position', 'Estimated', 'Error', 'Anchors'});
    xlabel('X (m)'); ylabel('Y (m)');
    title('DLS Trilateration - No Calibration');
    grid on; axis equal;
    ...
    % Save figure
    output_file = fullfile(figures_dir, 'pos_dls.jpg');
